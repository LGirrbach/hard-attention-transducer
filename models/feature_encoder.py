...
class FeatureEncoder(nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int, hidden_size: int, num_layers: int, dropout: float,
                 pooling: str, context_dim: int, device=torch.device):
        ...
        # Make embedder
        self.embedder = Embedder(vocab_size=vocab_size, embedding_dim=embedding_dim, dropout=dropout)

        # Make encoder
        if num_layers > 0:
            self.encoder = BiLSTMEncoder(
                input_size=embedding_dim, hidden_size=hidden_size, dropout=dropout, num_layers=num_layers
            )
        else:
    ...
    def forward(self, features: Tensor, feature_lengths: Tensor, contexts: Tensor) -> Tensor:
        ...
        if self.pooling_name in ["max", "mean", "sum"]:
            return self._timewise_pooling(encoded_features, feature_lengths, contexts)
        else:
            return self._attention_pooling(encoded_features, feature_lengths, contexts)
